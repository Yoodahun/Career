...
public class WorkORM
{
  ...
  public static Entity select(int id) throws ClassNotFoundException, SQLException
  {
    /*　IDで探す。1件しかReturnしない　*/
    StringBuffer sql = new StringBuffer();
    sql.append("SELECT ");
  ...
  public static Entity insert(String name) throws SQLException, ClassNotFoundException
  {
    ...
    sql.append("VALUES ");
    sql.append("( ");
    sql.append(" SEQ_WORK.nextval, ? ");
    sql.append(")");

    int id = OracleClient.insert(sql, name);
    return select(id);
